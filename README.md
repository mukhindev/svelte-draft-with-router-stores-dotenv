...
Dotenv добавляет переменных в окружение из файлов .env, описанных по схеме `КЛЮЧ=значение`.

Создадим в корне пару файлов: `.env.development` и `.env.production`. Хорошим тоном будет `.env.production` добавить в `.gitignore`.

В `.env.development` добавим:

```
API=http://127.0.0.1:3000/api
```

В `.env.production` добавим с тем же ключом `API`, но другим значением – адресом вашего продакшен сервера.

Далее работаем с `rollup.config.js`. Тут задаются параметры для сборщика **Rollup**.

Ранее мы добавили плагин `@rollup/plugin-replace` позволяющий заменять строки в собираемом проекте.

Подключаем плагин `replace` и модуль `dotenv.js`, извлекая нужную переменную `__API__`.
...
При сборке проекта любая строка `'__API__'` заменится на значение из соответствующего `.env` файла.

Теперь можно смело писать так: 
